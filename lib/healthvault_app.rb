...
module HealthvaultApp
  ...
  def self.get_patient_info(id, connection)
  ...
  def self.get_medications(id,connection)
        get_things(["5C5F1223-F63C-4464-870C-3E36BA471DEF"], id, connection)
  end
  
  def self.get_allergies(id,connection)
      get_things(["52bf9104-2c5e-4f1f-a66d-552ebcc53df7"], id, connection)
  end
  
  def self.get_results(id,connection)
      types = ["7b2ea78c-4b78-4f75-a6a7-5396fe38b09a", #aerobic profile
        ...
        get_things(types,id,connection)
  end
  
  def self.get_immunizations(id, connection)
      get_things(["3d817dbe-af42-4a9d-a553-d1298b4d08fc"], id, connection)
  end
  
  def self.get_advance_directives(id,connection)
    get_things(["822a5e5a-14f1-4d06-b92f-8f3f1b05218f"],id,connection)
  end
  
  def self.get_conditions(id,connection)
    get_things(["7ea7a1f9-880b-4bd4-b593-f5660f20eda8"], id , connection)
  end
  
  
  def self.get_insurance_plan(id,connection)
    get_things(["9366440c-ec81-4b89-b231-308a4c4d70ed"], id, connection)
  end
  
  def self.get_procedures(id,connection)
    get_things(["0A5F9A43-DC88-4E9F-890F-1F9159B76E7B"],id,connection)
  end
  
  
  def self.get_encounters(id,connection)
    get_things(["3D4BDF01-1B3E-4AFC-B41C-BD3E641A6DA7"],id,connection)
  end
  
  
  def self.get_emmergancey_contact(id,connection)
     get_things(["25c94a9f-9d3d-4576-96dc-6791178a8143"],id,connection)  
  end
  
  
  def self.get_things(types,id,connection)   
