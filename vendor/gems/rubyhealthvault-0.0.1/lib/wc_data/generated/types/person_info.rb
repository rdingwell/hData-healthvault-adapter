...
#<b>remarks</b>: Each account in the Microsoft Health Service is assigned a unique identifier which is a 128-bit integer (16 bytes) and is used to identify the account when calling methods on the service.
...
#<b>remarks</b>: When a person uses an application for the first time they are required to select a health record to use with the application. The application is then authorized to use the application and the Microsoft Health Service remembers the selection and returns the health record unique identifier to the application. A person may chose to have multiple records authorized for one application but the application should use the selected record by default.
#<em>value</em> is a HealthVault::WCData::Types::Guid
