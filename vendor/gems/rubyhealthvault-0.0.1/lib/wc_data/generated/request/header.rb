...
#<b>remarks</b>: This hash ensures that the info section is not tampered with. If the request is not authenticated, then this section is not required or expected. That is, in the authenticated request case, the header is protected via the HMAC operation over the header element, thereby preventing tampering. If the header is not protected then the info-hash can be modified arbitrarily. The Microsoft Health Service will compute the corresponding hash and compare the results. If they are not the same, the request will fail.
#<em>value</em> is a HealthVault::WCData::Types::HashFinalized
        ...
        def initialize
          ...
          @children['app-id'] = {:name => 'app-id', :class => HealthVault::WCData::Types::Guid, :value => nil, :min => 1, :max => 1, :order => 5, :place => :element, :choice => 230008194 }
            
          
        
          
          @children['auth-session'] = {:name => 'auth-session', :class => HealthVault::WCData::Request::AuthenticatedSessionInfo, :value => nil, :min => 1, :max => 1, :order => 6, :place => :element, :choice => 230008194 }
